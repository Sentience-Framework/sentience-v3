...
class PgSQLDialect extends SQLDialect
{
    ...
    {
        $column->type = $column->type instanceof ColumnTypeEnum ? $column->type->value : $column->type;
        ...
        if ($column->generatedByDefaultAsIdentity && !$this->generatedByDefaultAsIdentity()) {
            $column->type = match (strtoupper($column->type)) {
                ...
                default => throw new QueryException('GENERATED BY DEFAULT AS IDENTITY columns must be an integer type')
            };
        }

        if (strtoupper($column->type) == ColumnTypeEnum::DATETIME->value) {
            $column->type = $typeIsUppercase ? 'TIMESTAMP' : 'timestamp';
        }
